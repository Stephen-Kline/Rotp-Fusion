...
- [ ] 16. Event notification UI — `active_events` array on `SimulationState` is the data surface; UI reads from it and sends `PlayerAction.resolve_event(event_id, choice_id)`; **data layer done**, scene implementation deferred (needs Godot editor UI work)
